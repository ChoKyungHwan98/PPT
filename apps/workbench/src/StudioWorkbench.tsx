...
export function StudioWorkbench() {
  ...
  const changeMode = (next: 'document' | 'presentation') => { setMode(next); setOutput(null); setCriticRun(null); };
  ...
  const decide = async (decision: 'ready' | 'reject') => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(value.error ?? '판단을 실행 기록에 연결하지 못했습니다.');
      }
      await appendEvaluationEvent(event);
  ...
  return <div className="studio-workbench">
    ...
    <main className="studio-grid">
      <section className="source-pane"><header><h2>기획 원고</h2><div><select aria-label="자료 목적" value={mode} onChange={(event) => changeMode(event.target.value as 'document' | 'presentation')}><option value="document">기획서</option><option value="presentation">발표자료</option></select><select aria-label="정보 구조" value={structure} onChange={(event) => changeStructure(event.target.value as keyof typeof samples)}><option value="hierarchy">역할 구조</option><option value="aligned-before-after-spec">전후 비교</option></select></div></header><textarea value={content} onChange={(event) => setContent(event.target.value)} spellCheck={false}/><p>들여쓰기와 항목 대응이 정보 관계로 사용됩니다. 원문은 임의로 바꾸지 않습니다.</p></section>
