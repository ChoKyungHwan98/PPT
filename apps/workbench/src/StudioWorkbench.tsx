...
function ModelsModule({ registry, message, onActivate, onRollback, onBack }: { registry: TrainedModels | null; message: string; onActivate: (modelId: string) => void; onRollback: () => void; onBack: () => void }) {
  ...
  return <div className="product-shell"><header className="product-top"><button className="crumb" onClick={onBack}>‹ 도로시아</button><b>모델 관리</b><span>모든 프로젝트가 함께 사용하는 모델</span></header><main className="module-overview model-module"><header><h1>모델 관리</h1><p>학습 완료, 품질 검증, 실제 사용을 구분합니다. 검증 전 모델은 장표 검토에 쓰이지 않습니다.</p></header><div className="model-summary"><span><small>등록 모델</small><b>{registry?.models.length ?? '—'}</b></span><span><small>현재 사용</small><b>{registry?.activeVisualCriticModelId ?? '없음'}</b></span><button disabled={!registry?.rollbackStack.length} onClick={onRollback}>이전 모델로 되돌리기</button></div><section className="model-list">{registry?.models.map((model) => <article key={model.modelId} className={model.active ? 'is-active' : ''}><header><div><small>{model.version}</small><h2>{model.displayName}</h2><p>{model.baseModel}</p></div><strong>{statusName(model.benchmarkStatus)}</strong></header><dl><dt>학습 자료</dt><dd>{model.trainingDatasetId}</dd><dt>생성일</dt><dd>{new Date(model.createdAt).toLocaleString('ko-KR')}</dd><dt>평가 결과</dt><dd>{model.benchmark ? `문제 발견 ${(model.benchmark.findingRecall * 100).toFixed(0)}% · 준비 판정 ${(model.benchmark.readinessAccuracy * 100).toFixed(0)}%` : '아직 평가하지 않음'}</dd></dl><button disabled={model.benchmarkStatus !== 'qualified' || model.active} onClick={() => onActivate(model.modelId)}>{model.active ? '현재 사용 중' : '이 모델 사용'}</button></article>)}</section><p className="model-message">{message}</p><button className="module-back" onClick={onBack}>프로젝트로 돌아가기</button></main></div>;
}
...
export function StudioWorkbench() {
  ...
  if (screen === 'models') return <ModelsModule registry={trainedModels} message={message} onActivate={(modelId) => void activateModel(modelId)} onRollback={() => void rollbackModel()} onBack={() => setScreen('project')}/>;
